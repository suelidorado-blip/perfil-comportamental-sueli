...
const behavioralBlocks=[
  {kind:'disc',title:'Perfil Comportamental (DISC)',subtitle:'Como você tende a agir, decidir e responder a desafios.'},
...
export default function TestClient({assessment}:{assessment:any}){
  ...
  const scaleHelp=isBehavioral?'1 = pouco parecido comigo, 5 = muito parecido comigo.':step===0?'1 = não gostaria / não me atrai, 5 = gostaria muito / me atrai bastante.':'1 = pouco, 5 = muito.';
  return <main className="assessment-shell"><div className="assessment-wrap">{brand}<div className="card progress-card"><div className="progress-head"><div><p className="eyebrow">ETAPA {step+1} DE {blocks.length}</p><h1 style={{margin:'0 0 5px'}}>{block.title}</h1><p className="muted" style={{margin:0}}>{block.subtitle}</p></div><span className="badge">{progress}% concluído</span></div><div className="progress-track"><div className="progress-fill" style={{width:`${progress}%`}}/></div><p className="muted" style={{fontSize:12,marginBottom:0}}>Escala de 1 a 5: {scaleHelp}</p></div><form onSubmit={submit} className="grid">{currentQuestions.map((q,i)=><Question key={q.id} q={q} i={i} answers={answers} setAnswers={setAnswers}/>)}<div className="assessment-actions">{step>0?<button type="button" className="btn btn-ghost" onClick={()=>{setStep(s=>s-1);window.scrollTo({top:0,behavior:'smooth'})}}>← Voltar</button>:<span/>}{!isLast?<button type="button" className="btn btn-primary" onClick={nextBlock}>Continuar →</button>:<button className="btn btn-primary" disabled={sending}>{sending?'Enviando...':'Finalizar avaliação'}</button>}</div></form></div></main>;
}
function Question({q,i,answers,setAnswers}:{q:Q,i:number,answers:Record<string,string>,setAnswers:(v:Record<string,string>)=>void}){return <div className="question"><b>{i+1}. {q.text}</b><div style={{display:'flex',gap:18,flexWrap:'wrap',marginTop:12}}>{q.options.map(o=><label key={o.label} className="option"><input type="radio" name={q.id} checked={answers[q.id]===o.value} onChange={()=>setAnswers({...answers,[q.id]:o.value})}/>{o.label}</label>)}</div></div>}
