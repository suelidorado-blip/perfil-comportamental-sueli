...
export default async function TestPage({params}:{params:Promise<{token:string}>}){
  ...
  const {data}=await a.from('assessments').select('id,participant_name,respondent_name,type,status,context,token,expires_at,used_at,deleted_at,started_at').eq('token',token).maybeSingle();
